...
"""
    ...
    def read_system_config(self):
        """Parse and store the system config settings in electrum-tpc.conf into system_config[]."""
        name = '/etc/electrum-tpc.conf'
        if os.path.exists(name):
            try:
                import ConfigParser
            except ImportError:
                print "cannot parse electrum-tpc.conf. please install ConfigParser"
                return
